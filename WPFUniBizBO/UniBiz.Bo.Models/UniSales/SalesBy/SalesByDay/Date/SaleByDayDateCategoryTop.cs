...
// Type: UniBiz.Bo.Models.UniSales.SalesBy.SalesByDay.Date.SaleByDayDateCategoryTop
...
{
  public class SaleByDayDateCategoryTop : SaleByDayDateDept<SaleByDayDateCategoryTop>
  {
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new SaleByDayDateCategoryTop();

    public override object Clone()
    {
      SaleByDayDateCategoryTop dayDateCategoryTop = base.Clone() as SaleByDayDateCategoryTop;
      dayDateCategoryTop.ctg_lv1_ID = this.ctg_lv1_ID;
      dayDateCategoryTop.ctg_lv1_ViewCode = this.ctg_lv1_ViewCode;
      dayDateCategoryTop.ctg_lv1_Name = this.ctg_lv1_Name;
      return (object) dayDateCategoryTop;
    }

    public void PutData(SaleByDayDateCategoryTop pSource)
    {
      this.PutData((SaleByDayDateDept) pSource);
      this.ctg_lv1_ID = pSource.ctg_lv1_ID;
      this.ctg_lv1_ViewCode = pSource.ctg_lv1_ViewCode;
      this.ctg_lv1_Name = pSource.ctg_lv1_Name;
    }

    public bool IsZero(EnumZeroCheck pCheckType, SaleByDayDateCategoryTop pSource) => pSource == null || this.IsZero(pCheckType, (SaleByDayDateDept) pSource);
    ...
    public async Task<IList<SaleByDayDateCategoryTop>> SelectDateCategoryTopListAsync(object p_param)
    {
      SaleByDayDateCategoryTop dayDateCategoryTop1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(dayDateCategoryTop1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, dayDateCategoryTop1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(dayDateCategoryTop1.GetSelectQuery(p_param)))
        {
          dayDateCategoryTop1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + dayDateCategoryTop1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<SaleByDayDateCategoryTop>) null;
        }
        IList<SaleByDayDateCategoryTop> lt_list = (IList<SaleByDayDateCategoryTop>) new List<SaleByDayDateCategoryTop>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            SaleByDayDateCategoryTop dayDateCategoryTop2 = dayDateCategoryTop1.OleDB.Create<SaleByDayDateCategoryTop>();
            if (dayDateCategoryTop2.GetFieldValues(rs))
            {
              dayDateCategoryTop2.row_number = lt_list.Count + 1;
              lt_list.Add(dayDateCategoryTop2);
            }
          }
    ...
    public async IAsyncEnumerable<SaleByDayDateCategoryTop> SelectDateCategoryTopEnumerableAsync(
      object p_param)
    {
      SaleByDayDateCategoryTop dayDateCategoryTop1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      ...
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (dayDateCategoryTop2.GetFieldValues(rs))
            {
              dayDateCategoryTop2.row_number = ++row_number;
              yield return dayDateCategoryTop2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
