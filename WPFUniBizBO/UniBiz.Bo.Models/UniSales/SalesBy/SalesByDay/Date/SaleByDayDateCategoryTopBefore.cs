...
// Type: UniBiz.Bo.Models.UniSales.SalesBy.SalesByDay.Date.SaleByDayDateCategoryTopBefore
...
{
  public class SaleByDayDateCategoryTopBefore : 
    SaleByDayDateDeptBefore<SaleByDayDateCategoryTopBefore>
  {
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new SaleByDayDateCategoryTopBefore();

    public override object Clone()
    {
      SaleByDayDateCategoryTopBefore categoryTopBefore = base.Clone() as SaleByDayDateCategoryTopBefore;
      categoryTopBefore.ctg_lv1_ID = this.ctg_lv1_ID;
      categoryTopBefore.ctg_lv1_ViewCode = this.ctg_lv1_ViewCode;
      categoryTopBefore.ctg_lv1_Name = this.ctg_lv1_Name;
      return (object) categoryTopBefore;
    }

    public void PutData(SaleByDayDateCategoryTopBefore pSource)
    {
      this.PutData((SaleByDayDateDeptBefore) pSource);
      this.ctg_lv1_ID = pSource.ctg_lv1_ID;
      this.ctg_lv1_ViewCode = pSource.ctg_lv1_ViewCode;
      this.ctg_lv1_Name = pSource.ctg_lv1_Name;
    }

    public bool IsZero(EnumZeroCheck pCheckType, SaleByDayDateCategoryTopBefore pSource) => pSource == null || this.IsZero(pCheckType, (SaleByDayDateDeptBefore) pSource);
    ...
    public async Task<IList<SaleByDayDateCategoryTopBefore>> SelectDateCategoryTopBeforeListAsync(
      object p_param)
    {
      SaleByDayDateCategoryTopBefore categoryTopBefore1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(categoryTopBefore1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, categoryTopBefore1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(categoryTopBefore1.GetSelectQuery(p_param)))
        {
          categoryTopBefore1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + categoryTopBefore1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<SaleByDayDateCategoryTopBefore>) null;
        }
        IList<SaleByDayDateCategoryTopBefore> lt_list = (IList<SaleByDayDateCategoryTopBefore>) new List<SaleByDayDateCategoryTopBefore>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            SaleByDayDateCategoryTopBefore categoryTopBefore2 = categoryTopBefore1.OleDB.Create<SaleByDayDateCategoryTopBefore>();
            if (categoryTopBefore2.GetFieldValues(rs))
            {
              categoryTopBefore2.row_number = lt_list.Count + 1;
              lt_list.Add(categoryTopBefore2);
            }
          }
    ...
    public async IAsyncEnumerable<SaleByDayDateCategoryTopBefore> SelectDateCategoryTopBeforeEnumerableAsync(
      object p_param)
    {
      SaleByDayDateCategoryTopBefore categoryTopBefore1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      ...
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (categoryTopBefore2.GetFieldValues(rs))
            {
              categoryTopBefore2.row_number = ++row_number;
              yield return categoryTopBefore2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
    ...
    public override string GetSelectQuery(object p_param)
    {
      ...
      try
      {
        ...
        stringBuilder.Append(string.Format(" FROM {0}{1} {2}", (object) DbQueryHelper.ToDBNameBridge(UbModelBase.UNI_SALES), (object) TableCodeType.GoalByCategory, (object) DbQueryHelper.ToWithNolock()) + "\n INNER JOIN T_VIEW_CATEGORY_LV1  ON gbc_CategoryCode=T_VIEW_CATEGORY_LV1.ctg_lv1_ID AND gbc_SiteID=T_VIEW_CATEGORY_LV1.ctg_SiteID");
        p_param1.Clear();
        p_param1 = this.SearchConditionGoalByCategory((Hashtable) p_param);
        if (p_param1.Count > 0)
        {
        ...
        stringBuilder.Append(",dpt_lv1_ID,dpt_lv2_ID,dept_code");
        stringBuilder.Append(",ctg_lv1_ID");
        stringBuilder.Append(SaleByDayDateStoreBefore.BaseGoalCategoryColMakerBefore);
        ...
        p_param1.Clear();
        p_param1 = this.SearchConditionGoalByCategoryBefore((Hashtable) p_param);
        if (p_param1.Count > 0)
        {
          if (!p_param1.ContainsKey((object) "gbc_SiteID"))
            p_param1.Add((object) "gbc_SiteID", (object) p_SiteID);
