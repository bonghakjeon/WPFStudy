...
// Type: UniBiz.Bo.Models.UniSales.SalesBy.SalesByDay.Supplier.SaleByDayDateSupplierGoods
...
{
  public class SaleByDayDateSupplierGoods : 
    SaleByDayDateSupplierCategoryBot<SaleByDayDateSupplierGoods>
  {
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new SaleByDayDateSupplierGoods();

    public override object Clone()
    {
      SaleByDayDateSupplierGoods dateSupplierGoods = base.Clone() as SaleByDayDateSupplierGoods;
      dateSupplierGoods.gd_GoodsName = this.gd_GoodsName;
      dateSupplierGoods.gd_BarCode = this.gd_BarCode;
      dateSupplierGoods.gd_GoodsSize = this.gd_GoodsSize;
      dateSupplierGoods.gd_UseYn = this.gd_UseYn;
      dateSupplierGoods.gdh_BuyCost = this.gdh_BuyCost;
      dateSupplierGoods.gdh_BuyVat = this.gdh_BuyVat;
      dateSupplierGoods.gdh_SalePrice = this.gdh_SalePrice;
      dateSupplierGoods.gdh_EventCost = this.gdh_EventCost;
      dateSupplierGoods.gdh_EventVat = this.gdh_EventVat;
      dateSupplierGoods.gdh_EventPrice = this.gdh_EventPrice;
      return (object) dateSupplierGoods;
    }

    public void PutData(SaleByDayDateSupplierGoods pSource)
    {
      this.PutData((SaleByDayDateSupplierCategoryBot) pSource);
    ...
    public bool IsZero(EnumZeroCheck pCheckType, SaleByDayDateSupplierGoods pSource) => pSource == null || this.IsZero(pCheckType, (SaleByDayDateSupplierCategoryBot) pSource);
    ...
    public async Task<IList<SaleByDayDateSupplierGoods>> SelectDateSupplierGoodsListAsync(
      object p_param)
    {
      SaleByDayDateSupplierGoods dateSupplierGoods1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(dateSupplierGoods1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, dateSupplierGoods1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(dateSupplierGoods1.GetSelectQuery(p_param)))
        {
          dateSupplierGoods1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + dateSupplierGoods1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<SaleByDayDateSupplierGoods>) null;
        }
        IList<SaleByDayDateSupplierGoods> lt_list = (IList<SaleByDayDateSupplierGoods>) new List<SaleByDayDateSupplierGoods>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            SaleByDayDateSupplierGoods dateSupplierGoods2 = dateSupplierGoods1.OleDB.Create<SaleByDayDateSupplierGoods>();
            if (dateSupplierGoods2.GetFieldValues(rs))
            {
              dateSupplierGoods2.row_number = lt_list.Count + 1;
              lt_list.Add(dateSupplierGoods2);
            }
          }
    ...
    public async IAsyncEnumerable<SaleByDayDateSupplierGoods> SelectDateSupplierGoodsEnumerableAsync(
      object p_param)
    {
      SaleByDayDateSupplierGoods dateSupplierGoods1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      ...
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (dateSupplierGoods2.GetFieldValues(rs))
            {
              dateSupplierGoods2.row_number = ++row_number;
              yield return dateSupplierGoods2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
