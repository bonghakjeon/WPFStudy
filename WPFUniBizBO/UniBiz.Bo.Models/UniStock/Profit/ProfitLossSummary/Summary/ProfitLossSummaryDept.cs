...
// Type: UniBiz.Bo.Models.UniStock.Profit.ProfitLossSummary.Summary.ProfitLossSummaryDept
...
{
  public class ProfitLossSummaryDept : ProfitLossSummaryTeam<ProfitLossSummaryDept>
  {
    private int _dpt_ID;
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new ProfitLossSummaryDept();

    public override object Clone()
    {
      ProfitLossSummaryDept profitLossSummaryDept = base.Clone() as ProfitLossSummaryDept;
      profitLossSummaryDept.dpt_ID = this.dpt_ID;
      profitLossSummaryDept.dpt_ViewCode = this.dpt_ViewCode;
      profitLossSummaryDept.dpt_DeptName = this.dpt_DeptName;
      return (object) profitLossSummaryDept;
    }

    public void PutData(ProfitLossSummaryDept pSource)
    {
      this.PutData((ProfitLossSummaryTeam) pSource);
      this.dpt_ID = pSource.dpt_ID;
      this.dpt_ViewCode = pSource.dpt_ViewCode;
      this.dpt_DeptName = pSource.dpt_DeptName;
    }

    public bool IsZero(EnumZeroCheck pCheckType, ProfitLossSummaryDept pSource) => pSource == null || this.IsZero(pCheckType, (ProfitLossSummaryTeam) pSource);
    ...
    public async Task<IList<ProfitLossSummaryDept>> SelectDeptListAsync(object p_param)
    {
      ProfitLossSummaryDept profitLossSummaryDept1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(profitLossSummaryDept1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, profitLossSummaryDept1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(profitLossSummaryDept1.GetSelectQuery(p_param)))
        {
          profitLossSummaryDept1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + profitLossSummaryDept1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<ProfitLossSummaryDept>) null;
        }
        IList<ProfitLossSummaryDept> lt_list = (IList<ProfitLossSummaryDept>) new List<ProfitLossSummaryDept>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            ProfitLossSummaryDept profitLossSummaryDept2 = profitLossSummaryDept1.OleDB.Create<ProfitLossSummaryDept>();
            if (profitLossSummaryDept2.GetFieldValues(rs))
            {
              profitLossSummaryDept2.row_number = lt_list.Count + 1;
              lt_list.Add(profitLossSummaryDept2);
            }
          }
    ...
    public async IAsyncEnumerable<ProfitLossSummaryDept> SelectDeptEnumerableAsync(object p_param)
    {
      ProfitLossSummaryDept profitLossSummaryDept1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (profitLossSummaryDept2.GetFieldValues(rs))
            {
              profitLossSummaryDept2.row_number = ++row_number;
              yield return profitLossSummaryDept2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
