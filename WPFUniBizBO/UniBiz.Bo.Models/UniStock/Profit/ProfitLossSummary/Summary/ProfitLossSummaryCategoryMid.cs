...
// Type: UniBiz.Bo.Models.UniStock.Profit.ProfitLossSummary.Summary.ProfitLossSummaryCategoryMid
...
{
  public class ProfitLossSummaryCategoryMid : 
    ProfitLossSummaryCategoryTop<ProfitLossSummaryCategoryMid>
  {
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new ProfitLossSummaryCategoryMid();

    public override object Clone()
    {
      ProfitLossSummaryCategoryMid summaryCategoryMid = base.Clone() as ProfitLossSummaryCategoryMid;
      summaryCategoryMid.ctg_lv2_ID = this.ctg_lv2_ID;
      summaryCategoryMid.ctg_lv2_ViewCode = this.ctg_lv2_ViewCode;
      summaryCategoryMid.ctg_lv2_Name = this.ctg_lv2_Name;
      return (object) summaryCategoryMid;
    }

    public void PutData(ProfitLossSummaryCategoryMid pSource)
    {
      this.PutData((ProfitLossSummaryCategoryTop) pSource);
      this.ctg_lv2_ID = pSource.ctg_lv2_ID;
      this.ctg_lv2_ViewCode = pSource.ctg_lv2_ViewCode;
      this.ctg_lv2_Name = pSource.ctg_lv2_Name;
    }

    public bool IsZero(EnumZeroCheck pCheckType, ProfitLossSummaryCategoryMid pSource) => pSource == null || this.IsZero(pCheckType, (ProfitLossSummaryCategoryTop) pSource);
    ...
    public async Task<IList<ProfitLossSummaryCategoryMid>> SelectCategoryMidListAsync(object p_param)
    {
      ProfitLossSummaryCategoryMid summaryCategoryMid1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(summaryCategoryMid1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, summaryCategoryMid1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(summaryCategoryMid1.GetSelectQuery(p_param)))
        {
          summaryCategoryMid1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + summaryCategoryMid1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<ProfitLossSummaryCategoryMid>) null;
        }
        IList<ProfitLossSummaryCategoryMid> lt_list = (IList<ProfitLossSummaryCategoryMid>) new List<ProfitLossSummaryCategoryMid>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            ProfitLossSummaryCategoryMid summaryCategoryMid2 = summaryCategoryMid1.OleDB.Create<ProfitLossSummaryCategoryMid>();
            if (summaryCategoryMid2.GetFieldValues(rs))
            {
              summaryCategoryMid2.row_number = lt_list.Count + 1;
              lt_list.Add(summaryCategoryMid2);
            }
          }
    ...
    public async IAsyncEnumerable<ProfitLossSummaryCategoryMid> SelectCategoryMidEnumerableAsync(
      object p_param)
    {
      ProfitLossSummaryCategoryMid summaryCategoryMid1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      ...
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (summaryCategoryMid2.GetFieldValues(rs))
            {
              summaryCategoryMid2.row_number = ++row_number;
              yield return summaryCategoryMid2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
