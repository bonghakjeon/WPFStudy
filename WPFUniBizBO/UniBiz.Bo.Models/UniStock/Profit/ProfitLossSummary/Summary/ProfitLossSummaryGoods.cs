...
// Type: UniBiz.Bo.Models.UniStock.Profit.ProfitLossSummary.Summary.ProfitLossSummaryGoods
...
{
  public class ProfitLossSummaryGoods : ProfitLossSummaryCategoryBot<ProfitLossSummaryGoods>
  {
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new ProfitLossSummaryGoods();

    public override object Clone()
    {
      ProfitLossSummaryGoods lossSummaryGoods = base.Clone() as ProfitLossSummaryGoods;
      lossSummaryGoods.gd_GoodsName = this.gd_GoodsName;
      lossSummaryGoods.gd_BarCode = this.gd_BarCode;
      lossSummaryGoods.gd_GoodsSize = this.gd_GoodsSize;
      lossSummaryGoods.gd_UseYn = this.gd_UseYn;
      lossSummaryGoods.gdh_BuyCost = this.gdh_BuyCost;
      lossSummaryGoods.gdh_BuyVat = this.gdh_BuyVat;
      lossSummaryGoods.gdh_SalePrice = this.gdh_SalePrice;
      lossSummaryGoods.gdh_EventCost = this.gdh_EventCost;
      lossSummaryGoods.gdh_EventVat = this.gdh_EventVat;
      lossSummaryGoods.gdh_EventPrice = this.gdh_EventPrice;
      return (object) lossSummaryGoods;
    }

    public void PutData(ProfitLossSummaryGoods pSource)
    {
      this.PutData((ProfitLossSummaryCategoryBot) pSource);
    ...
    public bool IsZero(EnumZeroCheck pCheckType, ProfitLossSummaryGoods pSource) => pSource == null || this.IsZero(pCheckType, (ProfitLossSummaryCategoryBot) pSource);
    ...
    public async Task<IList<ProfitLossSummaryGoods>> SelectGoodsListAsync(object p_param)
    {
      ProfitLossSummaryGoods lossSummaryGoods1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(lossSummaryGoods1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, lossSummaryGoods1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(lossSummaryGoods1.GetSelectQuery(p_param)))
        {
          lossSummaryGoods1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + lossSummaryGoods1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<ProfitLossSummaryGoods>) null;
        }
        IList<ProfitLossSummaryGoods> lt_list = (IList<ProfitLossSummaryGoods>) new List<ProfitLossSummaryGoods>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            ProfitLossSummaryGoods lossSummaryGoods2 = lossSummaryGoods1.OleDB.Create<ProfitLossSummaryGoods>();
            if (lossSummaryGoods2.GetFieldValues(rs))
            {
              lossSummaryGoods2.row_number = lt_list.Count + 1;
              lt_list.Add(lossSummaryGoods2);
            }
          }
    ...
    public async IAsyncEnumerable<ProfitLossSummaryGoods> SelectGoodsEnumerableAsync(object p_param)
    {
      ProfitLossSummaryGoods lossSummaryGoods1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (lossSummaryGoods2.GetFieldValues(rs))
            {
              lossSummaryGoods2.row_number = ++row_number;
              yield return lossSummaryGoods2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
