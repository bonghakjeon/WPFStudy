...
// Type: UniBiz.Bo.Models.UniSales.SalesBy.SalesByDay.Date.SaleByDayDateDeptBefore
...
{
  public class SaleByDayDateDeptBefore : SaleByDayDateTeamBefore<SaleByDayDateDeptBefore>
  {
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new SaleByDayDateDeptBefore();

    public override object Clone()
    {
      SaleByDayDateDeptBefore dayDateDeptBefore = base.Clone() as SaleByDayDateDeptBefore;
      dayDateDeptBefore.dpt_ViewCode = this.dpt_ViewCode;
      dayDateDeptBefore.dpt_DeptName = this.dpt_DeptName;
      return (object) dayDateDeptBefore;
    }

    public void PutData(SaleByDayDateDeptBefore pSource)
    {
      this.PutData((SaleByDayDateTeamBefore) pSource);
      this.dpt_ViewCode = pSource.dpt_ViewCode;
      this.dpt_DeptName = pSource.dpt_DeptName;
    }

    public bool IsZero(EnumZeroCheck pCheckType, SaleByDayDateDeptBefore pSource) => pSource == null || this.IsZero(pCheckType, (SaleByDayDateTeamBefore) pSource);
    ...
    public async Task<IList<SaleByDayDateDeptBefore>> SelectDateDeptBeforeListAsync(object p_param)
    {
      SaleByDayDateDeptBefore dayDateDeptBefore1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(dayDateDeptBefore1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, dayDateDeptBefore1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(dayDateDeptBefore1.GetSelectQuery(p_param)))
        {
          dayDateDeptBefore1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + dayDateDeptBefore1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<SaleByDayDateDeptBefore>) null;
        }
        IList<SaleByDayDateDeptBefore> lt_list = (IList<SaleByDayDateDeptBefore>) new List<SaleByDayDateDeptBefore>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            SaleByDayDateDeptBefore dayDateDeptBefore2 = dayDateDeptBefore1.OleDB.Create<SaleByDayDateDeptBefore>();
            if (dayDateDeptBefore2.GetFieldValues(rs))
            {
              dayDateDeptBefore2.row_number = lt_list.Count + 1;
              lt_list.Add(dayDateDeptBefore2);
            }
          }
    ...
    public async IAsyncEnumerable<SaleByDayDateDeptBefore> SelectDateDeptBeforeEnumerableAsync(
      object p_param)
    {
      SaleByDayDateDeptBefore dayDateDeptBefore1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      ...
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (dayDateDeptBefore2.GetFieldValues(rs))
            {
              dayDateDeptBefore2.row_number = ++row_number;
              yield return dayDateDeptBefore2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
