...
// Type: UniBiz.Bo.Models.UniSales.SalesBy.SalesByDay.Diff.SalesByDayDiffTeam
...
{
  public class SalesByDayDiffTeam : SalesByDayDiffStore<SalesByDayDiffTeam>
  {
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new SalesByDayDiffTeam();

    public override object Clone()
    {
      SalesByDayDiffTeam salesByDayDiffTeam = base.Clone() as SalesByDayDiffTeam;
      salesByDayDiffTeam.dpt_lv1_ID = this.dpt_lv1_ID;
      salesByDayDiffTeam.dpt_lv1_ViewCode = this.dpt_lv1_ViewCode;
      salesByDayDiffTeam.dpt_lv1_Name = this.dpt_lv1_Name;
      salesByDayDiffTeam.dpt_lv2_ID = this.dpt_lv2_ID;
      salesByDayDiffTeam.dpt_lv2_ViewCode = this.dpt_lv2_ViewCode;
      salesByDayDiffTeam.dpt_lv2_Name = this.dpt_lv2_Name;
      return (object) salesByDayDiffTeam;
    }

    public void PutData(SalesByDayDiffTeam pSource)
    {
      this.PutData((SalesByDayDiffStore) pSource);
    ...
    public bool IsZero(EnumZeroCheck pCheckType, SalesByDayDiffTeam pSource) => pSource == null || this.IsZero(pCheckType, (SalesByDayDiffStore) pSource);
    ...
    public async Task<IList<SalesByDayDiffTeam>> SelectDateTeamListAsync(object p_param)
    {
      SalesByDayDiffTeam salesByDayDiffTeam1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(salesByDayDiffTeam1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, salesByDayDiffTeam1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(salesByDayDiffTeam1.GetSelectQuery(p_param)))
        {
          salesByDayDiffTeam1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + salesByDayDiffTeam1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<SalesByDayDiffTeam>) null;
        }
        IList<SalesByDayDiffTeam> lt_list = (IList<SalesByDayDiffTeam>) new List<SalesByDayDiffTeam>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            SalesByDayDiffTeam salesByDayDiffTeam2 = salesByDayDiffTeam1.OleDB.Create<SalesByDayDiffTeam>();
            if (salesByDayDiffTeam2.GetFieldValues(rs))
            {
              salesByDayDiffTeam2.row_number = lt_list.Count + 1;
              lt_list.Add(salesByDayDiffTeam2);
            }
          }
    ...
    public async IAsyncEnumerable<SalesByDayDiffTeam> SelectDateTeamEnumerableAsync(object p_param)
    {
      SalesByDayDiffTeam salesByDayDiffTeam1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (salesByDayDiffTeam2.GetFieldValues(rs))
            {
              salesByDayDiffTeam2.row_number = ++row_number;
              yield return salesByDayDiffTeam2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
