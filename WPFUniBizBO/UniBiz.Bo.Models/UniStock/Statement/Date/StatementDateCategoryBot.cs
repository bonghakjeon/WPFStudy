...
// Type: UniBiz.Bo.Models.UniStock.Statement.Date.StatementDateCategoryBot
...
{
  public class StatementDateCategoryBot : StatementDateCategoryMid<StatementDateCategoryBot>
  {
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new StatementDateCategoryBot();

    public override object Clone()
    {
      StatementDateCategoryBot statementDateCategoryBot = base.Clone() as StatementDateCategoryBot;
      statementDateCategoryBot.ctg_ViewCode = this.ctg_ViewCode;
      statementDateCategoryBot.ctg_CategoryName = this.ctg_CategoryName;
      return (object) statementDateCategoryBot;
    }

    public void PutData(StatementDateCategoryBot pSource)
    {
      this.PutData((StatementDateCategoryMid) pSource);
      this.ctg_ViewCode = pSource.ctg_ViewCode;
      this.ctg_CategoryName = pSource.ctg_CategoryName;
    }

    public bool IsZero(EnumZeroCheck pCheckType, StatementDateCategoryBot pSource) => pSource == null || this.IsZero(pCheckType, (StatementDateCategoryMid) pSource);
    ...
    public async Task<IList<StatementDateCategoryBot>> SelectDateCategoryBotListAsync(object p_param)
    {
      StatementDateCategoryBot statementDateCategoryBot1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(statementDateCategoryBot1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, statementDateCategoryBot1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(statementDateCategoryBot1.GetSelectQuery(p_param)))
        {
          statementDateCategoryBot1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + statementDateCategoryBot1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<StatementDateCategoryBot>) null;
        }
        IList<StatementDateCategoryBot> lt_list = (IList<StatementDateCategoryBot>) new List<StatementDateCategoryBot>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            StatementDateCategoryBot statementDateCategoryBot2 = statementDateCategoryBot1.OleDB.Create<StatementDateCategoryBot>();
            if (statementDateCategoryBot2.GetFieldValues(rs))
            {
              statementDateCategoryBot2.row_number = lt_list.Count + 1;
              lt_list.Add(statementDateCategoryBot2);
            }
          }
    ...
    public async IAsyncEnumerable<StatementDateCategoryBot> SelectDateCategoryBotEnumerableAsync(
      object p_param)
    {
      StatementDateCategoryBot statementDateCategoryBot1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      ...
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (statementDateCategoryBot2.GetFieldValues(rs))
            {
              statementDateCategoryBot2.row_number = ++row_number;
              yield return statementDateCategoryBot2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
