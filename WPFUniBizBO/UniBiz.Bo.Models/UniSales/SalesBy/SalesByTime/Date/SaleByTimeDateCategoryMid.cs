...
// Type: UniBiz.Bo.Models.UniSales.SalesBy.SalesByTime.Date.SaleByTimeDateCategoryMid
...
{
  public class SaleByTimeDateCategoryMid : SaleByTimeDateCategoryTop<SaleByTimeDateCategoryMid>
  {
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new SaleByTimeDateCategoryMid();

    public override object Clone()
    {
      SaleByTimeDateCategoryMid timeDateCategoryMid = base.Clone() as SaleByTimeDateCategoryMid;
      timeDateCategoryMid.ctg_lv2_ID = this.ctg_lv2_ID;
      timeDateCategoryMid.ctg_lv2_ViewCode = this.ctg_lv2_ViewCode;
      timeDateCategoryMid.ctg_lv2_Name = this.ctg_lv2_Name;
      return (object) timeDateCategoryMid;
    }

    public void PutData(SaleByTimeDateCategoryMid pSource)
    {
      this.PutData((SaleByTimeDateCategoryTop) pSource);
      this.ctg_lv2_ID = pSource.ctg_lv2_ID;
      this.ctg_lv2_ViewCode = pSource.ctg_lv2_ViewCode;
      this.ctg_lv2_Name = pSource.ctg_lv2_Name;
    }

    public bool IsZero(EnumZeroCheck pCheckType, SaleByTimeDateCategoryMid pSource) => pSource == null || this.IsZero(pCheckType, (SaleByTimeDateCategoryTop) pSource);
    ...
    public async Task<IList<SaleByTimeDateCategoryMid>> SelectDateCategoryMidListAsync(
      object p_param)
    {
      SaleByTimeDateCategoryMid timeDateCategoryMid1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(timeDateCategoryMid1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, timeDateCategoryMid1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(timeDateCategoryMid1.GetSelectQuery(p_param)))
        {
          timeDateCategoryMid1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + timeDateCategoryMid1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<SaleByTimeDateCategoryMid>) null;
        }
        IList<SaleByTimeDateCategoryMid> lt_list = (IList<SaleByTimeDateCategoryMid>) new List<SaleByTimeDateCategoryMid>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            SaleByTimeDateCategoryMid timeDateCategoryMid2 = timeDateCategoryMid1.OleDB.Create<SaleByTimeDateCategoryMid>();
            if (timeDateCategoryMid2.GetFieldValues(rs))
            {
              timeDateCategoryMid2.row_number = lt_list.Count + 1;
              lt_list.Add(timeDateCategoryMid2);
            }
          }
    ...
    public async IAsyncEnumerable<SaleByTimeDateCategoryMid> SelectDateCategoryMidEnumerableAsync(
      object p_param)
    {
      SaleByTimeDateCategoryMid timeDateCategoryMid1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      ...
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (timeDateCategoryMid2.GetFieldValues(rs))
            {
              timeDateCategoryMid2.row_number = ++row_number;
              yield return timeDateCategoryMid2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
