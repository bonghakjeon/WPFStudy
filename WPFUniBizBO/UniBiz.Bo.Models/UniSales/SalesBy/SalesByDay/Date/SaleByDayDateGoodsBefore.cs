...
// Type: UniBiz.Bo.Models.UniSales.SalesBy.SalesByDay.Date.SaleByDayDateGoodsBefore
...
{
  public class SaleByDayDateGoodsBefore : SaleByDayDateCategoryBotBefore<SaleByDayDateGoodsBefore>
  {
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new SaleByDayDateGoodsBefore();

    public override object Clone()
    {
      SaleByDayDateGoodsBefore dayDateGoodsBefore = base.Clone() as SaleByDayDateGoodsBefore;
      dayDateGoodsBefore.gd_GoodsName = this.gd_GoodsName;
      dayDateGoodsBefore.gd_BarCode = this.gd_BarCode;
      dayDateGoodsBefore.gd_GoodsSize = this.gd_GoodsSize;
      dayDateGoodsBefore.gd_UseYn = this.gd_UseYn;
      dayDateGoodsBefore.gdh_BuyCost = this.gdh_BuyCost;
      dayDateGoodsBefore.gdh_BuyVat = this.gdh_BuyVat;
      dayDateGoodsBefore.gdh_SalePrice = this.gdh_SalePrice;
      dayDateGoodsBefore.gdh_EventCost = this.gdh_EventCost;
      dayDateGoodsBefore.gdh_EventVat = this.gdh_EventVat;
      dayDateGoodsBefore.gdh_EventPrice = this.gdh_EventPrice;
      return (object) dayDateGoodsBefore;
    }

    public void PutData(SaleByDayDateGoodsBefore pSource)
    {
      this.PutData((SaleByDayDateCategoryBotBefore) pSource);
    ...
    public bool IsZero(EnumZeroCheck pCheckType, SaleByDayDateGoodsBefore pSource) => pSource == null || this.IsZero(pCheckType, (SaleByDayDateCategoryBotBefore) pSource);
    ...
    public async Task<IList<SaleByDayDateGoodsBefore>> SelectDateGoodsBeforeListAsync(object p_param)
    {
      SaleByDayDateGoodsBefore dayDateGoodsBefore1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(dayDateGoodsBefore1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, dayDateGoodsBefore1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(dayDateGoodsBefore1.GetSelectQuery(p_param)))
        {
          dayDateGoodsBefore1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + dayDateGoodsBefore1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<SaleByDayDateGoodsBefore>) null;
        }
        IList<SaleByDayDateGoodsBefore> lt_list = (IList<SaleByDayDateGoodsBefore>) new List<SaleByDayDateGoodsBefore>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            SaleByDayDateGoodsBefore dayDateGoodsBefore2 = dayDateGoodsBefore1.OleDB.Create<SaleByDayDateGoodsBefore>();
            if (dayDateGoodsBefore2.GetFieldValues(rs))
            {
              dayDateGoodsBefore2.row_number = lt_list.Count + 1;
              lt_list.Add(dayDateGoodsBefore2);
            }
          }
    ...
    public async IAsyncEnumerable<SaleByDayDateGoodsBefore> SelectDateGoodsBeforeEnumerableAsync(
      object p_param)
    {
      SaleByDayDateGoodsBefore dayDateGoodsBefore1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      ...
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (dayDateGoodsBefore2.GetFieldValues(rs))
            {
              dayDateGoodsBefore2.row_number = ++row_number;
              yield return dayDateGoodsBefore2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
