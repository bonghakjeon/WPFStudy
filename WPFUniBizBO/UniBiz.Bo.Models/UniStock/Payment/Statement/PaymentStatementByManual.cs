...
// Type: UniBiz.Bo.Models.UniStock.Payment.Statement.PaymentStatementByManual
...
{
  public class PaymentStatementByManual : PaymentView<PaymentStatementByManual>
  {
    ...
    public PaymentStatementByManual()
    {
    }

    public PaymentStatementByManual(Action<string> p_Callback)
      : this()
    {
    ...
    {
      PaymentStatementByManual statementByManual = this;
      try
      {
        if (statementByManual.db_status == 0)
        {
          if (!await statementByManual.InsertAsync())
            throw new UniServiceException(statementByManual.message, statementByManual.TableCode.ToDescription() + " 등록 오류");
        }
        else if (statementByManual.db_status > 0)
        {
          if (!await statementByManual.UpdateAsync((UbModelBase) null))
            throw new UniServiceException(statementByManual.message, statementByManual.TableCode.ToDescription() + " 등록 오류");
        }
        return true;
      }
      ...
      {
        statementByManual.message = " " + statementByManual.TableCode.ToDescription() + " 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + statementByManual.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n 내용 : " + ex.Message + "\n--------------------------------------------------------------------------------------------------\n";
        Log.Logger.ErrorColor(statementByManual.message);
      }
      return false;
    }
    ...
    {
      PaymentStatementByManual statementByManual = this;
      bool isMessage = statementByManual._CallBackMessage != null && pJobInfo != null;
      try
      {
        statementByManual.pay_SiteID = p_sh_SiteID;
        statementByManual.pay_StoreCode = p_sh_StoreCode;
        statementByManual.pay_Supplier = p_sh_Supplier;
        statementByManual.sh_ConfirmDate = new DateTime?(p_sh_ConfirmDate);
        if (statementByManual.DataCheck() != EnumDataCheck.Success)
          throw new Exception("작업 취소 처리.\n" + statementByManual.message);
        Log.Logger.DebugColor("\n전표 수기 결제 마감 " + pWorkDesc + " 해당 수기 결제 데이터 검색중 잠시만 ......\n시간이 오래 걸리는 작업 입니다.");
        IList<PaymentStatement> paymentStatementList = await statementByManual.SelectStatementListAsync();
        ...
        {
          PaymentStatement item = paymentStatement;
          if (pJobInfo != null && pJobInfo.IsWorkCancel)
            throw new Exception("작업 취소 처리.");
          item.SetAdoDatabase(statementByManual.OleDB);
          if (item.pay_SiteID == 0L)
            item.pay_SiteID = statementByManual.pay_SiteID;
          if (item.IsConfirm)
            throw new UniServiceException(statementByManual.TableCode.ToDescription() + " 재고 마감 오류", "[결제일자:" + new DateTime?(item.pay_Date.Value).GetDateToStr("yyyy-MM-dd") + "]일 [결제]마감 [" + item.su_SupplierName + "]거래처 입니다.");
          if (item.IsStatementConfirm)
            throw new UniServiceException(statementByManual.TableCode.ToDescription() + " 재고 마감 오류", "[결제일자:" + new DateTime?(item.pay_Date.Value).GetDateToStr("yyyy-MM-dd") + "]일 [매입전표]마감 [" + item.su_SupplierName + "]거래처 입니다.");
          if (item.pay_ConfirmAfterCount > 0)
            throw new UniServiceException(statementByManual.TableCode.ToDescription() + " 재고 마감 오류", "[결제일자:" + new DateTime?(item.pay_Date.Value).GetDateToStr("yyyy-MM-dd") + "]일 이후 [결제]마감 " + item.pay_ConfirmAfterCount.ToString("n0") + "건 등록된 [" + item.su_SupplierName + "]거래처 입니다.");
          if (item.pay_StatementConfirmAfterCount > 0)
            throw new UniServiceException(statementByManual.TableCode.ToDescription() + " 재고 마감 오류", "[결제일자:" + new DateTime?(item.pay_Date.Value).GetDateToStr("yyyy-MM-dd") + "]일 이후 [매입전표]마감 " + item.pay_StatementConfirmAfterCount.ToString("n0") + "건 등록된 [" + item.su_SupplierName + "]거래처 입니다.");
          ...
          if (item.pay_Code != 0L && (item.db_status != 0 || item.db_status == 0 && item.IsZero(EnumZeroCheck.ALL)))
          {
          ...
          if (viewPro != num)
          {
            ...
            if (isMessage)
            {
              ...
              statementByManual._CallBackMessage(jobEvtProgressing2.ToJson<JobEvtProgressing>());
            }
            else
      ...
      EmployeeView pEmployee)
    {
      ...
      try
      {
        ...
        foreach (PaymentStatement paymentStatement in (IEnumerable<PaymentStatement>) paymentStatementList)
        {
          ...
          if (paymentStatement.pay_Code != 0L && (paymentStatement.db_status != 0 || paymentStatement.db_status == 0 && paymentStatement.IsZero(EnumZeroCheck.ALL)) && !paymentStatement.ItemSave())
            return false;
          ++num1;
    ...
    public async Task<IList<PaymentStatement>> SelectStatementListAsync()
    {
      ...
      try
      {
        db = new UniOleDatabase(statementByManual.OleDB.ConnectionUrl, pCommandTimeout: 180);
        rs = new UniOleDbRecordset(db, statementByManual.OleDB.CommandTimeout);
        if (statementByManual.DataCheck() != EnumDataCheck.Success)
        {
          Log.Logger.ErrorColor("\n-" + statementByManual.TableCode.ToDescription() + " 작업 데이터 체크\n--------------------------------------------------------------------------------------------------\n" + statementByManual.message + "\n--------------------------------------------------------------------------------------------------");
          Exception exception = new Exception(statementByManual.message);
        }
        if (!await rs.OpenAsync(statementByManual.GetSelectQuery((object) new Hashtable()
        {
          {
            (object) "pay_SiteID",
            (object) statementByManual.pay_SiteID
          },
          {
            (object) "pay_StoreCode",
            (object) statementByManual.pay_StoreCode
          },
          {
            (object) "pay_Supplier",
            (object) statementByManual.pay_Supplier
          },
          {
            (object) "sh_ConfirmDate",
            (object) statementByManual.sh_ConfirmDate.Value.ToLastDateOfMonth()
          }
        })))
        {
          statementByManual.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + statementByManual.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
        ...
        if (await rs.IsDataReadAsync())
        {
          do
          {
            PaymentStatement paymentStatement = statementByManual.OleDB.Create<PaymentStatement>();
            if (paymentStatement.GetFieldValues(rs))
            {
              paymentStatement.row_number = lt_list.Count + 1;
              lt_list.Add(paymentStatement);
            }
          }
    ...
    public async IAsyncEnumerable<PaymentStatement> SelectStatementEnumerableAsync()
    {
      ...
      try
      {
        db = new UniOleDatabase(statementByManual.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, statementByManual.OleDB.CommandTimeout);
        if (statementByManual.DataCheck() != EnumDataCheck.Success)
        {
        ...
        else if (!await rs.OpenAsync(statementByManual.GetSelectQuery((object) new Hashtable()
        {
          {
            (object) "pay_SiteID",
            (object) statementByManual.pay_SiteID
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (paymentStatement.GetFieldValues(rs))
            {
              paymentStatement.row_number = ++row_number;
              yield return paymentStatement;
            }
          }
          while (await rs.IsDataReadAsync());
        }
    ...
    public override string GetSelectQuery(object p_param)
    {
      ...
      try
      {
        ...
        PaymentWorkByManual paymentWorkByManual = new PaymentWorkByManual();
        ...
        if (nullable.HasValue)
          stringBuilder.Append(paymentWorkByManual.ToWithPayContionStatementQuery(p_param, UbModelBase.UNI_STOCK, p_SiteID, nullable.Value));
