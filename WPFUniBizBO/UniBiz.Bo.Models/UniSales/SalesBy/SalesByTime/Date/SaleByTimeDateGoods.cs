...
// Type: UniBiz.Bo.Models.UniSales.SalesBy.SalesByTime.Date.SaleByTimeDateGoods
...
{
  public class SaleByTimeDateGoods : SaleByTimeDateCategoryBot<SaleByTimeDateGoods>
  {
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new SaleByTimeDateGoods();

    public override object Clone()
    {
      SaleByTimeDateGoods saleByTimeDateGoods = base.Clone() as SaleByTimeDateGoods;
      saleByTimeDateGoods.gd_GoodsName = this.gd_GoodsName;
      saleByTimeDateGoods.gd_BarCode = this.gd_BarCode;
      saleByTimeDateGoods.gd_GoodsSize = this.gd_GoodsSize;
      saleByTimeDateGoods.gd_UseYn = this.gd_UseYn;
      saleByTimeDateGoods.gdh_BuyCost = this.gdh_BuyCost;
      saleByTimeDateGoods.gdh_BuyVat = this.gdh_BuyVat;
      saleByTimeDateGoods.gdh_SalePrice = this.gdh_SalePrice;
      saleByTimeDateGoods.gdh_EventCost = this.gdh_EventCost;
      saleByTimeDateGoods.gdh_EventVat = this.gdh_EventVat;
      saleByTimeDateGoods.gdh_EventPrice = this.gdh_EventPrice;
      return (object) saleByTimeDateGoods;
    }

    public void PutData(SaleByTimeDateGoods pSource)
    {
      this.PutData((SaleByTimeDateCategoryBot) pSource);
    ...
    public bool IsZero(EnumZeroCheck pCheckType, SaleByTimeDateGoods pSource) => pSource == null || this.IsZero(pCheckType, (SaleByTimeDateCategoryBot) pSource);
    ...
    public async Task<IList<SaleByTimeDateGoods>> SelectDateGoodsListAsync(object p_param)
    {
      SaleByTimeDateGoods saleByTimeDateGoods1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(saleByTimeDateGoods1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, saleByTimeDateGoods1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(saleByTimeDateGoods1.GetSelectQuery(p_param)))
        {
          saleByTimeDateGoods1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + saleByTimeDateGoods1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<SaleByTimeDateGoods>) null;
        }
        IList<SaleByTimeDateGoods> lt_list = (IList<SaleByTimeDateGoods>) new List<SaleByTimeDateGoods>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            SaleByTimeDateGoods saleByTimeDateGoods2 = saleByTimeDateGoods1.OleDB.Create<SaleByTimeDateGoods>();
            if (saleByTimeDateGoods2.GetFieldValues(rs))
            {
              saleByTimeDateGoods2.row_number = lt_list.Count + 1;
              lt_list.Add(saleByTimeDateGoods2);
            }
          }
    ...
    public async IAsyncEnumerable<SaleByTimeDateGoods> SelectDateGoodsEnumerableAsync(object p_param)
    {
      SaleByTimeDateGoods saleByTimeDateGoods1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (saleByTimeDateGoods2.GetFieldValues(rs))
            {
              saleByTimeDateGoods2.row_number = ++row_number;
              yield return saleByTimeDateGoods2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
      ...
      try
      {
        ...
        if (p_param is Hashtable hashtable)
        {
          ...
          if (hashtable.ContainsKey((object) "sbt_SaleDate"))
            p_day = new DateTime?((DateTime) hashtable[(object) "sbt_SaleDate"]);
          if (hashtable.ContainsKey((object) "sbt_SaleDate_END_"))
            p_day = new DateTime?((DateTime) hashtable[(object) "sbt_SaleDate_END_"]);
        }
        ...
        stringBuilder.Append(",sbt_GoodsCode");
        ...
        stringBuilder.Append("SELECT NULL AS sbt_SaleDate,sbt_StoreCode,gdh_GoodsCategory AS sbt_CategoryCode,T_DEPT_LV3_NAME.dpt_ID AS sbt_DeptCode,gd_BoxGoodsCode AS sbt_BoxCode,sbt_GoodsCode,gdh_Supplier AS sbt_Supplier,0 AS sbt_KeySeq" + string.Format(",{0} AS {1}", (object) p_SiteID, (object) "sbt_SiteID") + ",0 AS sbt_DayOfWeek,0 AS sbt_WeekOfYear,0 AS sbt_DayOfYear");
