...
// Type: UniBiz.Bo.Models.UniSales.SalesBy.SalesByDay.Diff.SalesByDayDiffCategoryBot
...
{
  public class SalesByDayDiffCategoryBot : SalesByDayDiffCategoryMid<SalesByDayDiffCategoryBot>
  {
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new SalesByDayDiffCategoryBot();

    public override object Clone()
    {
      SalesByDayDiffCategoryBot dayDiffCategoryBot = base.Clone() as SalesByDayDiffCategoryBot;
      dayDiffCategoryBot.ctg_ViewCode = this.ctg_ViewCode;
      dayDiffCategoryBot.ctg_CategoryName = this.ctg_CategoryName;
      return (object) dayDiffCategoryBot;
    }

    public void PutData(SalesByDayDiffCategoryBot pSource)
    {
      this.PutData((SalesByDayDiffCategoryMid) pSource);
      this.ctg_ViewCode = pSource.ctg_ViewCode;
      this.ctg_CategoryName = pSource.ctg_CategoryName;
    }

    public bool IsZero(EnumZeroCheck pCheckType, SalesByDayDiffCategoryBot pSource) => pSource == null || this.IsZero(pCheckType, (SalesByDayDiffCategoryMid) pSource);
    ...
    public async Task<IList<SalesByDayDiffCategoryBot>> SelectDateCategoryBotListAsync(
      object p_param)
    {
      SalesByDayDiffCategoryBot dayDiffCategoryBot1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(dayDiffCategoryBot1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, dayDiffCategoryBot1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(dayDiffCategoryBot1.GetSelectQuery(p_param)))
        {
          dayDiffCategoryBot1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + dayDiffCategoryBot1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<SalesByDayDiffCategoryBot>) null;
        }
        IList<SalesByDayDiffCategoryBot> lt_list = (IList<SalesByDayDiffCategoryBot>) new List<SalesByDayDiffCategoryBot>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            SalesByDayDiffCategoryBot dayDiffCategoryBot2 = dayDiffCategoryBot1.OleDB.Create<SalesByDayDiffCategoryBot>();
            if (dayDiffCategoryBot2.GetFieldValues(rs))
            {
              dayDiffCategoryBot2.row_number = lt_list.Count + 1;
              lt_list.Add(dayDiffCategoryBot2);
            }
          }
    ...
    public async IAsyncEnumerable<SalesByDayDiffCategoryBot> SelectDateCategoryBotEnumerableAsync(
      object p_param)
    {
      SalesByDayDiffCategoryBot dayDiffCategoryBot1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      ...
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (dayDiffCategoryBot2.GetFieldValues(rs))
            {
              dayDiffCategoryBot2.row_number = ++row_number;
              yield return dayDiffCategoryBot2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
