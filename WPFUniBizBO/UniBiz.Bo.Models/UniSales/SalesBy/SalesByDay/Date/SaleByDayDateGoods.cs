...
// Type: UniBiz.Bo.Models.UniSales.SalesBy.SalesByDay.Date.SaleByDayDateGoods
...
{
  public class SaleByDayDateGoods : SaleByDayDateCategoryBot<SaleByDayDateGoods>
  {
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new SaleByDayDateGoods();

    public override object Clone()
    {
      SaleByDayDateGoods saleByDayDateGoods = base.Clone() as SaleByDayDateGoods;
      saleByDayDateGoods.gd_GoodsName = this.gd_GoodsName;
      saleByDayDateGoods.gd_BarCode = this.gd_BarCode;
      saleByDayDateGoods.gd_GoodsSize = this.gd_GoodsSize;
      saleByDayDateGoods.gd_UseYn = this.gd_UseYn;
      saleByDayDateGoods.gdh_BuyCost = this.gdh_BuyCost;
      saleByDayDateGoods.gdh_BuyVat = this.gdh_BuyVat;
      saleByDayDateGoods.gdh_SalePrice = this.gdh_SalePrice;
      saleByDayDateGoods.gdh_EventCost = this.gdh_EventCost;
      saleByDayDateGoods.gdh_EventVat = this.gdh_EventVat;
      saleByDayDateGoods.gdh_EventPrice = this.gdh_EventPrice;
      return (object) saleByDayDateGoods;
    }

    public void PutData(SaleByDayDateGoods pSource)
    {
      this.PutData((SaleByDayDateCategoryBot) pSource);
    ...
    public bool IsZero(EnumZeroCheck pCheckType, SaleByDayDateGoods pSource) => pSource == null || this.IsZero(pCheckType, (SaleByDayDateCategoryBot) pSource);
    ...
    public async Task<IList<SaleByDayDateGoods>> SelectDateGoodsListAsync(object p_param)
    {
      SaleByDayDateGoods saleByDayDateGoods1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(saleByDayDateGoods1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, saleByDayDateGoods1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(saleByDayDateGoods1.GetSelectQuery(p_param)))
        {
          saleByDayDateGoods1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + saleByDayDateGoods1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<SaleByDayDateGoods>) null;
        }
        IList<SaleByDayDateGoods> lt_list = (IList<SaleByDayDateGoods>) new List<SaleByDayDateGoods>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            SaleByDayDateGoods saleByDayDateGoods2 = saleByDayDateGoods1.OleDB.Create<SaleByDayDateGoods>();
            if (saleByDayDateGoods2.GetFieldValues(rs))
            {
              saleByDayDateGoods2.row_number = lt_list.Count + 1;
              lt_list.Add(saleByDayDateGoods2);
            }
          }
    ...
    public async IAsyncEnumerable<SaleByDayDateGoods> SelectDateGoodsEnumerableAsync(object p_param)
    {
      SaleByDayDateGoods saleByDayDateGoods1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (saleByDayDateGoods2.GetFieldValues(rs))
            {
              saleByDayDateGoods2.row_number = ++row_number;
              yield return saleByDayDateGoods2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
