...
// Type: UniBiz.Bo.Models.UniStock.Profit.ProfitLossSummary.Date.ProfitLossSummaryDateTeam
...
{
  public class ProfitLossSummaryDateTeam : ProfitLossSummaryDateStore<ProfitLossSummaryDateTeam>
  {
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new ProfitLossSummaryDateTeam();

    public override object Clone()
    {
      ProfitLossSummaryDateTeam lossSummaryDateTeam = base.Clone() as ProfitLossSummaryDateTeam;
      lossSummaryDateTeam.dpt_lv1_ID = this.dpt_lv1_ID;
      lossSummaryDateTeam.dpt_lv1_ViewCode = this.dpt_lv1_ViewCode;
      lossSummaryDateTeam.dpt_lv1_Name = this.dpt_lv1_Name;
      lossSummaryDateTeam.dpt_lv2_ID = this.dpt_lv2_ID;
      lossSummaryDateTeam.dpt_lv2_ViewCode = this.dpt_lv2_ViewCode;
      lossSummaryDateTeam.dpt_lv2_Name = this.dpt_lv2_Name;
      return (object) lossSummaryDateTeam;
    }

    public void PutData(ProfitLossSummaryDateTeam pSource)
    {
      this.PutData((ProfitLossSummaryDateStore) pSource);
    ...
    public bool IsZero(EnumZeroCheck pCheckType, ProfitLossSummaryDateTeam pSource) => pSource == null || this.IsZero(pCheckType, (ProfitLossSummaryDateStore) pSource);
    ...
    public async Task<IList<ProfitLossSummaryDateTeam>> SelectDateTeamListAsync(object p_param)
    {
      ProfitLossSummaryDateTeam lossSummaryDateTeam1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(lossSummaryDateTeam1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, lossSummaryDateTeam1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(lossSummaryDateTeam1.GetSelectQuery(p_param)))
        {
          lossSummaryDateTeam1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + lossSummaryDateTeam1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<ProfitLossSummaryDateTeam>) null;
        }
        IList<ProfitLossSummaryDateTeam> lt_list = (IList<ProfitLossSummaryDateTeam>) new List<ProfitLossSummaryDateTeam>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            ProfitLossSummaryDateTeam lossSummaryDateTeam2 = lossSummaryDateTeam1.OleDB.Create<ProfitLossSummaryDateTeam>();
            if (lossSummaryDateTeam2.GetFieldValues(rs))
            {
              lossSummaryDateTeam2.row_number = lt_list.Count + 1;
              lt_list.Add(lossSummaryDateTeam2);
            }
          }
    ...
    public async IAsyncEnumerable<ProfitLossSummaryDateTeam> SelectDateTeamEnumerableAsync(
      object p_param)
    {
      ProfitLossSummaryDateTeam lossSummaryDateTeam1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      ...
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (lossSummaryDateTeam2.GetFieldValues(rs))
            {
              lossSummaryDateTeam2.row_number = ++row_number;
              yield return lossSummaryDateTeam2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
