...
// Type: UniBiz.Bo.Models.UniStock.Statement.Date.StatementDateTeam
...
{
  public class StatementDateTeam : StatementDateStore<StatementDateTeam>
  {
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new StatementDateTeam();

    public override object Clone()
    {
      StatementDateTeam statementDateTeam = base.Clone() as StatementDateTeam;
      statementDateTeam.dpt_lv1_ID = this.dpt_lv1_ID;
      statementDateTeam.dpt_lv1_ViewCode = this.dpt_lv1_ViewCode;
      statementDateTeam.dpt_lv1_Name = this.dpt_lv1_Name;
      statementDateTeam.dpt_lv2_ID = this.dpt_lv2_ID;
      statementDateTeam.dpt_lv2_ViewCode = this.dpt_lv2_ViewCode;
      statementDateTeam.dpt_lv2_Name = this.dpt_lv2_Name;
      return (object) statementDateTeam;
    }

    public void PutData(StatementDateTeam pSource)
    {
    ...
    public bool IsZero(EnumZeroCheck pCheckType, StatementDateTeam pSource) => pSource == null || this.IsZero(pCheckType, (StatementDateStore) pSource);
    ...
    public async Task<IList<StatementDateTeam>> SelectDateTeamListAsync(object p_param)
    {
      StatementDateTeam statementDateTeam1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(statementDateTeam1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, statementDateTeam1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(statementDateTeam1.GetSelectQuery(p_param)))
        {
          statementDateTeam1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + statementDateTeam1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<StatementDateTeam>) null;
        }
        IList<StatementDateTeam> lt_list = (IList<StatementDateTeam>) new List<StatementDateTeam>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            StatementDateTeam statementDateTeam2 = statementDateTeam1.OleDB.Create<StatementDateTeam>();
            if (statementDateTeam2.GetFieldValues(rs))
            {
              statementDateTeam2.row_number = lt_list.Count + 1;
              lt_list.Add(statementDateTeam2);
            }
          }
    ...
    public async IAsyncEnumerable<StatementDateTeam> SelectDateTeamEnumerableAsync(object p_param)
    {
      StatementDateTeam statementDateTeam1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (statementDateTeam2.GetFieldValues(rs))
            {
              statementDateTeam2.row_number = ++row_number;
              yield return statementDateTeam2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
