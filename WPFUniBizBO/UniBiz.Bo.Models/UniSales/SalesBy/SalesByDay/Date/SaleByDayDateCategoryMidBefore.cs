...
// Type: UniBiz.Bo.Models.UniSales.SalesBy.SalesByDay.Date.SaleByDayDateCategoryMidBefore
...
{
  public class SaleByDayDateCategoryMidBefore : 
    SaleByDayDateCategoryTopBefore<SaleByDayDateCategoryMidBefore>
  {
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new SaleByDayDateCategoryMidBefore();

    public override object Clone()
    {
      SaleByDayDateCategoryMidBefore categoryMidBefore = base.Clone() as SaleByDayDateCategoryMidBefore;
      categoryMidBefore.ctg_lv2_ID = this.ctg_lv2_ID;
      categoryMidBefore.ctg_lv2_ViewCode = this.ctg_lv2_ViewCode;
      categoryMidBefore.ctg_lv2_Name = this.ctg_lv2_Name;
      return (object) categoryMidBefore;
    }

    public void PutData(SaleByDayDateCategoryMidBefore pSource)
    {
      this.PutData((SaleByDayDateCategoryTopBefore) pSource);
      this.ctg_lv2_ID = pSource.ctg_lv2_ID;
      this.ctg_lv2_ViewCode = pSource.ctg_lv2_ViewCode;
      this.ctg_lv2_Name = pSource.ctg_lv2_Name;
    }

    public bool IsZero(EnumZeroCheck pCheckType, SaleByDayDateCategoryMidBefore pSource) => pSource == null || this.IsZero(pCheckType, (SaleByDayDateCategoryTopBefore) pSource);
    ...
    public async Task<IList<SaleByDayDateCategoryMidBefore>> SelectDateCategoryMidBeforeListAsync(
      object p_param)
    {
      SaleByDayDateCategoryMidBefore categoryMidBefore1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(categoryMidBefore1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, categoryMidBefore1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(categoryMidBefore1.GetSelectQuery(p_param)))
        {
          categoryMidBefore1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + categoryMidBefore1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<SaleByDayDateCategoryMidBefore>) null;
        }
        IList<SaleByDayDateCategoryMidBefore> lt_list = (IList<SaleByDayDateCategoryMidBefore>) new List<SaleByDayDateCategoryMidBefore>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            SaleByDayDateCategoryMidBefore categoryMidBefore2 = categoryMidBefore1.OleDB.Create<SaleByDayDateCategoryMidBefore>();
            if (categoryMidBefore2.GetFieldValues(rs))
            {
              categoryMidBefore2.row_number = lt_list.Count + 1;
              lt_list.Add(categoryMidBefore2);
            }
          }
    ...
    public async IAsyncEnumerable<SaleByDayDateCategoryMidBefore> SelectDateCategoryMidBeforeEnumerableAsync(
      object p_param)
    {
      SaleByDayDateCategoryMidBefore categoryMidBefore1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      ...
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (categoryMidBefore2.GetFieldValues(rs))
            {
              categoryMidBefore2.row_number = ++row_number;
              yield return categoryMidBefore2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
