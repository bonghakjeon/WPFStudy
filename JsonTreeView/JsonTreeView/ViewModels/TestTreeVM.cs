...
namespace JsonTreeView.ViewModels
{
    ...
    public class TestTreeVM : Screen
    {
        ...
        /// </summary>
        public List<ITreeNode> FolderList 
        { 
            get
            {
                // 싱글톤 패턴
                if (_FolderList == null)
                {
                    _FolderList = new List<ITreeNode>();
                }
                return _FolderList;
            }
            set => _FolderList = value;
        } 
        private List<ITreeNode> _FolderList;
        ...
        private async Task CreateListAsync(List<ProjectView> pProjectList, List<ITreeNode> pTeamList, List<ITreeNode> pFolderList, List<ITreeNode> pFileAndFolderList)
        {
            ...
            try
            {
                ...
                foreach (var data in datas)
                {
                    ...
                    var rootFolderList = folderList.Where(folder => folder["parentFolderId"] == null).Select(folder => new FolderView {        
                            ...
                            exp_TreeFolderId = folderId = folder[FolderHelper.folderId].ToString(),
                            exp_TreeName = folderName = folder[FolderHelper.folderName].ToString(),
                            exp_TreeFolderLevelNo = folderLevelNo = folder[FolderHelper.levelNo].ToString(),
                    ...
                    {
                        pFolderList.Add(rootFolder);

                        // FolderHelper.FillFolders(rootFolder, folderList);
                    ...
                    //    //    // pFolderList = new List<ITreeNode>();
                    ...
                    //    //    subNodes.Add(FolderInfo);

                    //    //    FolderInfo.ChildNodes = subNodes;

                    //    //    if (folderLevelNo.Equals(FolderHelper.rootFolder)) pFolderList.Add(FolderInfo);
                    //    //}
        ...
        public async Task FillFolders(FolderView parentFolder, List<Dictionary<string, object>> folderList)
        {
            try
            {
                // 서브 폴더 데이터 추출 
                var childNodes = folderList.Where(folder => parentFolder.exp_TreeFolderId == folder["parentFolderId"]?.ToString());

                if (childNodes.Any() == false) return;

                foreach (var childNode in childNodes)
                {
                    // TODO : Dictionary 특정 키(Key)에 해당하는 값(Value) 구해서 FolderView 클래스에 존재하는 프로퍼티 "", "", ""에 값 할당 (2023.09.15 jbh)
                    // 참고 URL - https://developer-talk.tistory.com/694
                    var subFolder = new FolderView {
                                            FolderDataInfo = ToDictionary(childNode),
                                            FolderItems = new List<Dictionary<string, object>> { childNode },
                                            exp_TreeParentFolderId = parentFolder.exp_TreeFolderId,
                                            exp_TreeFolderGroupId = childNode["folderGroupId"].ToString(),
                                            exp_TreeFolderId = childNode["folderId"].ToString(),
                                            exp_TreeName = childNode["folderName"].ToString(),
                                            exp_TreeFolderLevelNo = childNode["levelNo"].ToString(),
                                            exp_TreeProjectId = parentFolder.exp_TreeProjectId,
                                            exp_TreeTeamId = parentFolder.exp_TreeTeamId,
                    };

                    // 부모 폴더의 자식 노드로 서브폴더 추가 
                    parentFolder.ChildNodes.Add(subFolder);

                    await FillFolders(subFolder, folderList);
                }
            }
            catch (Exception e)
            {
