...
// Type: UniBiz.Bo.Models.UniStock.Inventory.InventorySummary.Date.InventorySummaryDateDept
...
{
  public class InventorySummaryDateDept : InventorySummaryDateTeam<InventorySummaryDateDept>
  {
    private int _dpt_ID;
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new InventorySummaryDateDept();

    public override object Clone()
    {
      InventorySummaryDateDept inventorySummaryDateDept = base.Clone() as InventorySummaryDateDept;
      inventorySummaryDateDept.dpt_ID = this.dpt_ID;
      inventorySummaryDateDept.dpt_ViewCode = this.dpt_ViewCode;
      inventorySummaryDateDept.dpt_DeptName = this.dpt_DeptName;
      return (object) inventorySummaryDateDept;
    }

    public void PutData(InventorySummaryDateDept pSource)
    {
      this.PutData((InventorySummaryDateTeam) pSource);
      this.dpt_ID = pSource.dpt_ID;
      this.dpt_ViewCode = pSource.dpt_ViewCode;
      this.dpt_DeptName = pSource.dpt_DeptName;
    }

    public bool IsZero(EnumZeroCheck pCheckType, InventorySummaryDateDept pSource) => pSource == null || this.IsZero(pCheckType, (InventorySummaryDateTeam) pSource);
    ...
    public async Task<IList<InventorySummaryDateDept>> SelectDateDeptListAsync(object p_param)
    {
      InventorySummaryDateDept inventorySummaryDateDept1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(inventorySummaryDateDept1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, inventorySummaryDateDept1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(inventorySummaryDateDept1.GetSelectQuery(p_param)))
        {
          inventorySummaryDateDept1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + inventorySummaryDateDept1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<InventorySummaryDateDept>) null;
        }
        IList<InventorySummaryDateDept> lt_list = (IList<InventorySummaryDateDept>) new List<InventorySummaryDateDept>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            InventorySummaryDateDept inventorySummaryDateDept2 = inventorySummaryDateDept1.OleDB.Create<InventorySummaryDateDept>();
            if (inventorySummaryDateDept2.GetFieldValues(rs))
            {
              inventorySummaryDateDept2.row_number = lt_list.Count + 1;
              lt_list.Add(inventorySummaryDateDept2);
            }
          }
    ...
    public async IAsyncEnumerable<InventorySummaryDateDept> SelectDateDeptEnumerableAsync(
      object p_param)
    {
      InventorySummaryDateDept inventorySummaryDateDept1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      ...
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (inventorySummaryDateDept2.GetFieldValues(rs))
            {
              inventorySummaryDateDept2.row_number = ++row_number;
              yield return inventorySummaryDateDept2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
