...
// Type: UniBiz.Bo.Models.UniSales.SalesBy.SalesByDay.Date.SaleByDayDateTeamBefore
...
{
  public class SaleByDayDateTeamBefore : SaleByDayDateStoreBefore<SaleByDayDateTeamBefore>
  {
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new SaleByDayDateTeamBefore();

    public override object Clone()
    {
      SaleByDayDateTeamBefore dayDateTeamBefore = base.Clone() as SaleByDayDateTeamBefore;
      dayDateTeamBefore.dpt_lv1_ID = this.dpt_lv1_ID;
      dayDateTeamBefore.dpt_lv1_ViewCode = this.dpt_lv1_ViewCode;
      dayDateTeamBefore.dpt_lv1_Name = this.dpt_lv1_Name;
      dayDateTeamBefore.dpt_lv2_ID = this.dpt_lv2_ID;
      dayDateTeamBefore.dpt_lv2_ViewCode = this.dpt_lv2_ViewCode;
      dayDateTeamBefore.dpt_lv2_Name = this.dpt_lv2_Name;
      return (object) dayDateTeamBefore;
    }

    public void PutData(SaleByDayDateTeamBefore pSource)
    {
      this.PutData((SaleByDayDateStoreBefore) pSource);
    ...
    public bool IsZero(EnumZeroCheck pCheckType, SaleByDayDateTeamBefore pSource) => pSource == null || this.IsZero(pCheckType, (SaleByDayDateStoreBefore) pSource);
    ...
    public async Task<IList<SaleByDayDateTeamBefore>> SelectDateTeamBeforeListAsync(object p_param)
    {
      SaleByDayDateTeamBefore dayDateTeamBefore1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(dayDateTeamBefore1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, dayDateTeamBefore1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(dayDateTeamBefore1.GetSelectQuery(p_param)))
        {
          dayDateTeamBefore1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + dayDateTeamBefore1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<SaleByDayDateTeamBefore>) null;
        }
        IList<SaleByDayDateTeamBefore> lt_list = (IList<SaleByDayDateTeamBefore>) new List<SaleByDayDateTeamBefore>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            SaleByDayDateTeamBefore dayDateTeamBefore2 = dayDateTeamBefore1.OleDB.Create<SaleByDayDateTeamBefore>();
            if (dayDateTeamBefore2.GetFieldValues(rs))
            {
              dayDateTeamBefore2.row_number = lt_list.Count + 1;
              lt_list.Add(dayDateTeamBefore2);
            }
          }
    ...
    public async IAsyncEnumerable<SaleByDayDateTeamBefore> SelectDateTeamBeforeEnumerableAsync(
      object p_param)
    {
      SaleByDayDateTeamBefore dayDateTeamBefore1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      ...
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (dayDateTeamBefore2.GetFieldValues(rs))
            {
              dayDateTeamBefore2.row_number = ++row_number;
              yield return dayDateTeamBefore2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
    ...
    {
      StringBuilder stringBuilder = new StringBuilder(new SaleByDayDateTeam().GetSelectWhereAnd(p_param, false));
