...
// Type: UniBiz.Bo.Models.UniStock.Statement.Supplier.StatementSupplierDate
...
{
  public class StatementSupplierDate : StatementDateStore<StatementSupplierDate>
  {
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new StatementSupplierDate();

    public override object Clone()
    {
      StatementSupplierDate statementSupplierDate = base.Clone() as StatementSupplierDate;
      statementSupplierDate.sh_Supplier = this.sh_Supplier;
      statementSupplierDate.su_HeadSupplier = this.su_HeadSupplier;
      statementSupplierDate.su_SupplierName = this.su_SupplierName;
      statementSupplierDate.su_SupplierViewCode = this.su_SupplierViewCode;
      statementSupplierDate.su_SupplierType = this.su_SupplierType;
      statementSupplierDate.su_SupplierKind = this.su_SupplierKind;
      statementSupplierDate.su_UseYn = this.su_UseYn;
      return (object) statementSupplierDate;
    }

    public void PutData(StatementSupplierDate pSource)
    {
      this.PutData((StatementDateStore) pSource);
    ...
    public bool IsZero(EnumZeroCheck pCheckType, StatementSupplierDate pSource) => pSource == null || this.IsZero(pCheckType, (StatementDateStore) pSource);
    ...
    public async Task<IList<StatementSupplierDate>> SelectSupplierDateListAsync(object p_param)
    {
      StatementSupplierDate statementSupplierDate1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(statementSupplierDate1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, statementSupplierDate1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(statementSupplierDate1.GetSelectQuery(p_param)))
        {
          statementSupplierDate1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + statementSupplierDate1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<StatementSupplierDate>) null;
        }
        IList<StatementSupplierDate> lt_list = (IList<StatementSupplierDate>) new List<StatementSupplierDate>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            StatementSupplierDate statementSupplierDate2 = statementSupplierDate1.OleDB.Create<StatementSupplierDate>();
            if (statementSupplierDate2.GetFieldValues(rs))
            {
              statementSupplierDate2.row_number = lt_list.Count + 1;
              lt_list.Add(statementSupplierDate2);
            }
          }
    ...
    public async IAsyncEnumerable<StatementSupplierDate> SelectSupplierDateEnumerableAsync(
      object p_param)
    {
      StatementSupplierDate statementSupplierDate1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      ...
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (statementSupplierDate2.GetFieldValues(rs))
            {
              statementSupplierDate2.row_number = ++row_number;
              yield return statementSupplierDate2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
