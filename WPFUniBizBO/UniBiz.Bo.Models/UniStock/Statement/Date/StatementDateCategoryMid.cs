...
// Type: UniBiz.Bo.Models.UniStock.Statement.Date.StatementDateCategoryMid
...
{
  public class StatementDateCategoryMid : StatementDateCategoryTop<StatementDateCategoryMid>
  {
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new StatementDateCategoryMid();

    public override object Clone()
    {
      StatementDateCategoryMid statementDateCategoryMid = base.Clone() as StatementDateCategoryMid;
      statementDateCategoryMid.ctg_lv2_ID = this.ctg_lv2_ID;
      statementDateCategoryMid.ctg_lv2_ViewCode = this.ctg_lv2_ViewCode;
      statementDateCategoryMid.ctg_lv2_Name = this.ctg_lv2_Name;
      return (object) statementDateCategoryMid;
    }

    public void PutData(StatementDateCategoryMid pSource)
    {
      this.PutData((StatementDateCategoryTop) pSource);
      this.ctg_lv2_ID = pSource.ctg_lv2_ID;
      this.ctg_lv2_ViewCode = pSource.ctg_lv2_ViewCode;
      this.ctg_lv2_Name = pSource.ctg_lv2_Name;
    }

    public bool IsZero(EnumZeroCheck pCheckType, StatementDateCategoryMid pSource) => pSource == null || this.IsZero(pCheckType, (StatementDateCategoryTop) pSource);
    ...
    public async Task<IList<StatementDateCategoryMid>> SelectDateCategoryMidListAsync(object p_param)
    {
      StatementDateCategoryMid statementDateCategoryMid1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(statementDateCategoryMid1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, statementDateCategoryMid1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(statementDateCategoryMid1.GetSelectQuery(p_param)))
        {
          statementDateCategoryMid1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + statementDateCategoryMid1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<StatementDateCategoryMid>) null;
        }
        IList<StatementDateCategoryMid> lt_list = (IList<StatementDateCategoryMid>) new List<StatementDateCategoryMid>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            StatementDateCategoryMid statementDateCategoryMid2 = statementDateCategoryMid1.OleDB.Create<StatementDateCategoryMid>();
            if (statementDateCategoryMid2.GetFieldValues(rs))
            {
              statementDateCategoryMid2.row_number = lt_list.Count + 1;
              lt_list.Add(statementDateCategoryMid2);
            }
          }
    ...
    public async IAsyncEnumerable<StatementDateCategoryMid> SelectDateCategoryMidEnumerableAsync(
      object p_param)
    {
      StatementDateCategoryMid statementDateCategoryMid1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      ...
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (statementDateCategoryMid2.GetFieldValues(rs))
            {
              statementDateCategoryMid2.row_number = ++row_number;
              yield return statementDateCategoryMid2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
