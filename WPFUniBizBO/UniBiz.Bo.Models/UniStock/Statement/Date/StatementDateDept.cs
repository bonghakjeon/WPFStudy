...
// Type: UniBiz.Bo.Models.UniStock.Statement.Date.StatementDateDept
...
{
  public class StatementDateDept : StatementDateTeam<StatementDateDept>
  {
    private int _dpt_ID;
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new StatementDateDept();

    public override object Clone()
    {
      StatementDateDept statementDateDept = base.Clone() as StatementDateDept;
      statementDateDept.dpt_ID = this.dpt_ID;
      statementDateDept.dpt_ViewCode = this.dpt_ViewCode;
      statementDateDept.dpt_DeptName = this.dpt_DeptName;
      return (object) statementDateDept;
    }

    public void PutData(StatementDateDept pSource)
    {
      this.PutData((StatementDateTeam) pSource);
      this.dpt_ID = pSource.dpt_ID;
      this.dpt_ViewCode = pSource.dpt_ViewCode;
      this.dpt_DeptName = pSource.dpt_DeptName;
    }

    public bool IsZero(EnumZeroCheck pCheckType, StatementDateDept pSource) => pSource == null || this.IsZero(pCheckType, (StatementDateTeam) pSource);
    ...
    public async Task<IList<StatementDateDept>> SelectDateDeptListAsync(object p_param)
    {
      StatementDateDept statementDateDept1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(statementDateDept1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, statementDateDept1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(statementDateDept1.GetSelectQuery(p_param)))
        {
          statementDateDept1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + statementDateDept1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<StatementDateDept>) null;
        }
        IList<StatementDateDept> lt_list = (IList<StatementDateDept>) new List<StatementDateDept>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            StatementDateDept statementDateDept2 = statementDateDept1.OleDB.Create<StatementDateDept>();
            if (statementDateDept2.GetFieldValues(rs))
            {
              statementDateDept2.row_number = lt_list.Count + 1;
              lt_list.Add(statementDateDept2);
            }
          }
    ...
    public async IAsyncEnumerable<StatementDateDept> SelectDateDeptEnumerableAsync(object p_param)
    {
      StatementDateDept statementDateDept1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (statementDateDept2.GetFieldValues(rs))
            {
              statementDateDept2.row_number = ++row_number;
              yield return statementDateDept2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
