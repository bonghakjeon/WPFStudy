...
// Type: UniBiz.Bo.Models.UniSales.SalesBy.SalesByDay.Month.Vertical.SaleByMonthVerticalByGoods
...
{
  public class SaleByMonthVerticalByGoods : 
    SaleByMonthVerticalByCategoryBot<SaleByMonthVerticalByGoods>
  {
    ...
    public override string ToString() => string.Format("{0} [{1}] Count : {2}", (object) nameof (SaleByMonthVerticalByGoods), (object) this.sbd_SaleDate, (object) this.Lt_Details.Count);
    ...
    public void InitInfo(SaleByMonthVerticalByGoods pData, IList<TGoods> p_Header)
    {
      ...
      foreach (TGoods tgoods in (IEnumerable<TGoods>) p_Header)
      {
        ...
        salesByMonthGoal.sbd_GoodsCode = tgoods.gd_GoodsCode;
        ltDetails.Add((SalesByDayGoal) salesByMonthGoal);
      }
    }

    public void Add(SaleByMonthVerticalByGoods item)
    {
    ...
    public void AddRange(IList<SaleByMonthVerticalByGoods> infos)
    {
      foreach (SaleByMonthVerticalByGoods info in (IEnumerable<SaleByMonthVerticalByGoods>) infos)
        this.Add(info);
    }

    public bool IsZero(EnumZeroCheck pCheckType, SaleByMonthVerticalByGoods pSource) => pSource == null || this.IsZero(pCheckType, (SaleByMonthVerticalByCategoryBot) pSource);
    ...
    public async Task<IList<SaleByMonthVerticalByGoods>> SelectMonthVerticalByGoodsListAsync(
      object p_param)
    {
      SaleByMonthVerticalByGoods monthVerticalByGoods1 = this;
      ...
      try
      {
        ...
        if (!await rs.OpenAsync(monthVerticalByGoods1.GetSelectQuery(p_param)))
        {
          ...
          return (IList<SaleByMonthVerticalByGoods>) null;
        }
        IList<SaleByMonthVerticalByGoods> lt_list = (IList<SaleByMonthVerticalByGoods>) new List<SaleByMonthVerticalByGoods>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            SaleByMonthVerticalByGoods monthVerticalByGoods2 = monthVerticalByGoods1.OleDB.Create<SaleByMonthVerticalByGoods>();
    ...
    public async IAsyncEnumerable<SaleByMonthVerticalByGoods> SelectMonthVerticalByGoodsEnumerableAsync(
      object p_param)
    {
      SaleByMonthVerticalByGoods monthVerticalByGoods1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
