...
// Type: UniBiz.Bo.Models.UniStock.Profit.ProfitLossSummary.Date.ProfitLossSummaryDateDept
...
{
  public class ProfitLossSummaryDateDept : ProfitLossSummaryDateTeam<ProfitLossSummaryDateDept>
  {
    private int _dpt_ID;
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new ProfitLossSummaryDateDept();

    public override object Clone()
    {
      ProfitLossSummaryDateDept lossSummaryDateDept = base.Clone() as ProfitLossSummaryDateDept;
      lossSummaryDateDept.dpt_ID = this.dpt_ID;
      lossSummaryDateDept.dpt_ViewCode = this.dpt_ViewCode;
      lossSummaryDateDept.dpt_DeptName = this.dpt_DeptName;
      return (object) lossSummaryDateDept;
    }

    public void PutData(ProfitLossSummaryDateDept pSource)
    {
      this.PutData((ProfitLossSummaryDateTeam) pSource);
      this.dpt_ID = pSource.dpt_ID;
      this.dpt_ViewCode = pSource.dpt_ViewCode;
      this.dpt_DeptName = pSource.dpt_DeptName;
    }

    public bool IsZero(EnumZeroCheck pCheckType, ProfitLossSummaryDateDept pSource) => pSource == null || this.IsZero(pCheckType, (ProfitLossSummaryDateTeam) pSource);
    ...
    public async Task<IList<ProfitLossSummaryDateDept>> SelectDateDeptListAsync(object p_param)
    {
      ProfitLossSummaryDateDept lossSummaryDateDept1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(lossSummaryDateDept1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, lossSummaryDateDept1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(lossSummaryDateDept1.GetSelectQuery(p_param)))
        {
          lossSummaryDateDept1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + lossSummaryDateDept1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<ProfitLossSummaryDateDept>) null;
        }
        IList<ProfitLossSummaryDateDept> lt_list = (IList<ProfitLossSummaryDateDept>) new List<ProfitLossSummaryDateDept>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            ProfitLossSummaryDateDept lossSummaryDateDept2 = lossSummaryDateDept1.OleDB.Create<ProfitLossSummaryDateDept>();
            if (lossSummaryDateDept2.GetFieldValues(rs))
            {
              lossSummaryDateDept2.row_number = lt_list.Count + 1;
              lt_list.Add(lossSummaryDateDept2);
            }
          }
    ...
    public async IAsyncEnumerable<ProfitLossSummaryDateDept> SelectDateDeptEnumerableAsync(
      object p_param)
    {
      ProfitLossSummaryDateDept lossSummaryDateDept1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      ...
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (lossSummaryDateDept2.GetFieldValues(rs))
            {
              lossSummaryDateDept2.row_number = ++row_number;
              yield return lossSummaryDateDept2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
