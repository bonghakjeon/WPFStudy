...
// Type: UniBiz.Bo.Models.UniSales.SalesBy.SalesByDay.Diff.SalesByDayDiffDept
...
{
  public class SalesByDayDiffDept : SalesByDayDiffTeam<SalesByDayDiffDept>
  {
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new SalesByDayDiffDept();

    public override object Clone()
    {
      SalesByDayDiffDept salesByDayDiffDept = base.Clone() as SalesByDayDiffDept;
      salesByDayDiffDept.dpt_ViewCode = this.dpt_ViewCode;
      salesByDayDiffDept.dpt_DeptName = this.dpt_DeptName;
      return (object) salesByDayDiffDept;
    }

    public void PutData(SalesByDayDiffDept pSource)
    {
      this.PutData((SalesByDayDiffTeam) pSource);
      this.dpt_ViewCode = pSource.dpt_ViewCode;
      this.dpt_DeptName = pSource.dpt_DeptName;
    }

    public bool IsZero(EnumZeroCheck pCheckType, SalesByDayDiffDept pSource) => pSource == null || this.IsZero(pCheckType, (SalesByDayDiffTeam) pSource);
    ...
    public async Task<IList<SalesByDayDiffDept>> SelectDateDeptListAsync(object p_param)
    {
      SalesByDayDiffDept salesByDayDiffDept1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(salesByDayDiffDept1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, salesByDayDiffDept1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(salesByDayDiffDept1.GetSelectQuery(p_param)))
        {
          salesByDayDiffDept1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + salesByDayDiffDept1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<SalesByDayDiffDept>) null;
        }
        IList<SalesByDayDiffDept> lt_list = (IList<SalesByDayDiffDept>) new List<SalesByDayDiffDept>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            SalesByDayDiffDept salesByDayDiffDept2 = salesByDayDiffDept1.OleDB.Create<SalesByDayDiffDept>();
            if (salesByDayDiffDept2.GetFieldValues(rs))
            {
              salesByDayDiffDept2.row_number = lt_list.Count + 1;
              lt_list.Add(salesByDayDiffDept2);
            }
          }
    ...
    public async IAsyncEnumerable<SalesByDayDiffDept> SelectDateDeptEnumerableAsync(object p_param)
    {
      SalesByDayDiffDept salesByDayDiffDept1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (salesByDayDiffDept2.GetFieldValues(rs))
            {
              salesByDayDiffDept2.row_number = ++row_number;
              yield return salesByDayDiffDept2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
