...
// Type: UniBiz.Bo.Models.UniStock.Statement.Month.Horizontal.Supplier.StatementMonthHorizontalBySupplierCatTop
...
{
  public class StatementMonthHorizontalBySupplierCatTop : 
    StatementMonthHorizontalBySupplier<StatementMonthHorizontalBySupplierCatTop>
  {
    private int _ctg_lv1_ID;
    private string _ctg_lv1_ViewCode;
    private string _ctg_lv1_Name;

    public override string KeyCode => string.Format("{0}|{1}|{2}", (object) this.sh_StoreCode, (object) this.sh_Supplier, (object) this.ctg_lv1_ID);
    ...
    public override string ToString() => string.Format("{0} [{1}({2})][{3}] Count : {4}", (object) nameof (StatementMonthHorizontalBySupplierCatTop), (object) this.su_SupplierName, (object) this.sh_Supplier, (object) this.ctg_lv1_Name, (object) this.Lt_Details.Count);
    ...
    public void PutInfo(StatementMonthHorizontalBySupplierCatTop pSource)
    {
      this.PutInfo((StatementMonthHorizontalBySupplier) pSource);
    ...
    public void InitInfo(StatementMonthHorizontalBySupplierCatTop pData, IList<DateTime> p_Days)
    {
      if (!this.KeyCode.Equals(pData.KeyCode))
        throw new Exception("pData 의 [sh_StoreCode|sh_Supplier|ctg_lv1_ID] 이 KeyCode 와 다릅니다");
      this.PutInfo(pData);
    ...
    public void Add(StatementMonthHorizontalBySupplierCatTop item)
    {
      ...
      else
      {
        ...
        if (!this.KeyCode.Equals(item.KeyCode))
          throw new Exception("item 의 [sh_StoreCode|sh_Supplier|ctg_lv1_ID] 이 KeyCode 와 다릅니다");
      }
    ...
    public void AddRange(
      IList<StatementMonthHorizontalBySupplierCatTop> infos)
    {
      foreach (StatementMonthHorizontalBySupplierCatTop info in (IEnumerable<StatementMonthHorizontalBySupplierCatTop>) infos)
        this.Add(info);
    }

    public bool IsZero(EnumZeroCheck pCheckType, StatementMonthHorizontalBySupplierCatTop pSource) => pSource == null || this.IsZero(pCheckType, (StatementMonthHorizontalBySupplier) pSource);
    ...
    public async Task<IList<StatementMonthHorizontalBySupplierCatTop>> SelectMonthHorizontalBySupplierCatTopListAsync(
      object p_param)
    {
      StatementMonthHorizontalBySupplierCatTop bySupplierCatTop1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(bySupplierCatTop1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, bySupplierCatTop1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(bySupplierCatTop1.GetSelectQuery(p_param)))
        {
          bySupplierCatTop1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + bySupplierCatTop1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<StatementMonthHorizontalBySupplierCatTop>) null;
        }
        IList<StatementMonthHorizontalBySupplierCatTop> lt_list = (IList<StatementMonthHorizontalBySupplierCatTop>) new List<StatementMonthHorizontalBySupplierCatTop>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            StatementMonthHorizontalBySupplierCatTop bySupplierCatTop2 = bySupplierCatTop1.OleDB.Create<StatementMonthHorizontalBySupplierCatTop>();
            if (bySupplierCatTop2.GetFieldValues(rs))
            {
              bySupplierCatTop2.row_number = lt_list.Count + 1;
              lt_list.Add(bySupplierCatTop2);
            }
          }
    ...
    public async IAsyncEnumerable<StatementMonthHorizontalBySupplierCatTop> SelectMonthHorizontalBySupplierCatTopEnumerableAsync(
      object p_param)
    {
      StatementMonthHorizontalBySupplierCatTop bySupplierCatTop1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      ...
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (bySupplierCatTop2.GetFieldValues(rs))
            {
              bySupplierCatTop2.row_number = ++row_number;
              yield return bySupplierCatTop2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
