...
// Type: UniBiz.Bo.Models.UniSales.SalesBy.SalesByDay.Day.Horizontal.Supplier.SalesByDayHorizontalBySupplierCategoryTop
...
{
  public class SalesByDayHorizontalBySupplierCategoryTop : 
    SalesByDayHorizontalBySupplier<SalesByDayHorizontalBySupplierCategoryTop>
  {
    private int _ctg_lv1_ID;
    private string _ctg_lv1_ViewCode;
    private string _ctg_lv1_Name;

    public override string KeyCode => string.Format("{0}|{1}|{2}", (object) this.si_StoreCode, (object) this.sbd_Supplier, (object) this.ctg_lv1_ID);
    ...
    public override string ToString() => string.Format("{0} [{1}({2})][{3}] Count : {4}", (object) nameof (SalesByDayHorizontalBySupplierCategoryTop), (object) this.su_SupplierName, (object) this.sbd_Supplier, (object) this.ctg_lv1_Name, (object) this.Lt_Details.Count);
    ...
    public void PutInfo(SalesByDayHorizontalBySupplierCategoryTop pSource)
    {
      this.PutInfo((SalesByDayHorizontalBySupplier) pSource);
    ...
    public void InitInfo(SalesByDayHorizontalBySupplierCategoryTop pData, IList<DateTime> p_Days)
    {
      if (!this.KeyCode.Equals(pData.KeyCode))
        throw new Exception("pData 의 [sbd_StoreCode|sbd_Supplier|ctg_lv1_ID] 이 KeyCode 와 다릅니다");
      this.PutInfo(pData);
      foreach (DateTime pDay in (IEnumerable<DateTime>) p_Days)
      {
        ...
        salesByDayGoal.sbd_Supplier = this.sbd_Supplier;
        ltDetails.Add(salesByDayGoal);
      }
    }

    public void Add(SalesByDayHorizontalBySupplierCategoryTop item)
    {
      if (this.si_StoreCode == 0)
      {
        ...
        throw new Exception("item 의 [sbd_StoreCode|sbd_Supplier|ctg_lv1_ID] 이 KeyCode 와 다릅니다");
    ...
    public void AddRange(
      IList<SalesByDayHorizontalBySupplierCategoryTop> infos)
    {
      foreach (SalesByDayHorizontalBySupplierCategoryTop info in (IEnumerable<SalesByDayHorizontalBySupplierCategoryTop>) infos)
        this.Add(info);
    }

    public bool IsZero(EnumZeroCheck pCheckType, SalesByDayHorizontalBySupplierCategoryTop pSource) => pSource == null || this.IsZero(pCheckType, (SalesByDayHorizontalBySupplier) pSource);
    ...
    public async Task<IList<SalesByDayHorizontalBySupplierCategoryTop>> SelectDayHorizontalBySupplierCategoryTopListAsync(
      object p_param)
    {
      SalesByDayHorizontalBySupplierCategoryTop supplierCategoryTop1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(supplierCategoryTop1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, supplierCategoryTop1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(supplierCategoryTop1.GetSelectQuery(p_param)))
        {
          supplierCategoryTop1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + supplierCategoryTop1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<SalesByDayHorizontalBySupplierCategoryTop>) null;
        }
        IList<SalesByDayHorizontalBySupplierCategoryTop> lt_list = (IList<SalesByDayHorizontalBySupplierCategoryTop>) new List<SalesByDayHorizontalBySupplierCategoryTop>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            SalesByDayHorizontalBySupplierCategoryTop supplierCategoryTop2 = supplierCategoryTop1.OleDB.Create<SalesByDayHorizontalBySupplierCategoryTop>();
            if (supplierCategoryTop2.GetFieldValues(rs))
            {
              supplierCategoryTop2.row_number = lt_list.Count + 1;
              lt_list.Add(supplierCategoryTop2);
            }
          }
    ...
    public async IAsyncEnumerable<SalesByDayHorizontalBySupplierCategoryTop> SelectDayHorizontalBySupplierCategoryTopEnumerableAsync(
      object p_param)
    {
      SalesByDayHorizontalBySupplierCategoryTop supplierCategoryTop1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      ...
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (supplierCategoryTop2.GetFieldValues(rs))
            {
              supplierCategoryTop2.row_number = ++row_number;
              yield return supplierCategoryTop2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
    ...
    public override string GetSelectQuery(object p_param)
    {
      ...
      try
      {
        ...
        stringBuilder.Append(",ctg_lv1_ID,T_CTG_LV_1_NAME.ctg_ViewCode AS ctg_lv1_ViewCode,T_CTG_LV_1_NAME.ctg_CategoryName AS ctg_lv1_Name");
        stringBuilder.Append("\nFROM (");
        stringBuilder.Append("\nSELECT sbd_SaleDate,sbd_StoreCode");
