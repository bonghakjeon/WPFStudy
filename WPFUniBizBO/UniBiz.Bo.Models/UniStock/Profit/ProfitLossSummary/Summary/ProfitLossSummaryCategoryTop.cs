...
// Type: UniBiz.Bo.Models.UniStock.Profit.ProfitLossSummary.Summary.ProfitLossSummaryCategoryTop
...
{
  public class ProfitLossSummaryCategoryTop : ProfitLossSummaryDept<ProfitLossSummaryCategoryTop>
  {
    ...
    protected override UbModelBase CreateClone => (UbModelBase) new ProfitLossSummaryCategoryTop();

    public override object Clone()
    {
      ProfitLossSummaryCategoryTop summaryCategoryTop = base.Clone() as ProfitLossSummaryCategoryTop;
      summaryCategoryTop.ctg_lv1_ID = this.ctg_lv1_ID;
      summaryCategoryTop.ctg_lv1_ViewCode = this.ctg_lv1_ViewCode;
      summaryCategoryTop.ctg_lv1_Name = this.ctg_lv1_Name;
      return (object) summaryCategoryTop;
    }

    public void PutData(ProfitLossSummaryCategoryTop pSource)
    {
      this.PutData((ProfitLossSummaryDept) pSource);
      this.ctg_lv1_ID = pSource.ctg_lv1_ID;
      this.ctg_lv1_ViewCode = pSource.ctg_lv1_ViewCode;
      this.ctg_lv1_Name = pSource.ctg_lv1_Name;
    }

    public bool IsZero(EnumZeroCheck pCheckType, ProfitLossSummaryCategoryTop pSource) => pSource == null || this.IsZero(pCheckType, (ProfitLossSummaryDept) pSource);
    ...
    public async Task<IList<ProfitLossSummaryCategoryTop>> SelectCategoryTopListAsync(object p_param)
    {
      ProfitLossSummaryCategoryTop summaryCategoryTop1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      UniOleDatabase db = (UniOleDatabase) null;
      try
      {
        db = new UniOleDatabase(summaryCategoryTop1.OleDB.ConnectionUrl);
        rs = new UniOleDbRecordset(db, summaryCategoryTop1.OleDB.CommandTimeout);
        if (!await rs.OpenAsync(summaryCategoryTop1.GetSelectQuery(p_param)))
        {
          summaryCategoryTop1.SetErrorInfo(rs.LastErrorID, "검색 오류 - SQL OPEN 실패\n--------------------------------------------------------------------------------------------------\n 테이블 : " + summaryCategoryTop1.TableCode.ToDescription() + "\n 메소드 : " + MethodBase.GetCurrentMethod().ReflectedType.Name + "==>" + MethodBase.GetCurrentMethod().Name + "\n" + string.Format(" LINE : {0} 행\n", (object) new StackFrame(0, true).GetFileLineNumber()) + "--------------------------------------------------------------------------------------------------\n" + string.Format(" 에러 : {0}\n", (object) rs.LastErrorID) + " 내용 : " + rs.LastErrorMessage + "\n--------------------------------------------------------------------------------------------------\n");
          return (IList<ProfitLossSummaryCategoryTop>) null;
        }
        IList<ProfitLossSummaryCategoryTop> lt_list = (IList<ProfitLossSummaryCategoryTop>) new List<ProfitLossSummaryCategoryTop>();
        if (await rs.IsDataReadAsync())
        {
          do
          {
            ProfitLossSummaryCategoryTop summaryCategoryTop2 = summaryCategoryTop1.OleDB.Create<ProfitLossSummaryCategoryTop>();
            if (summaryCategoryTop2.GetFieldValues(rs))
            {
              summaryCategoryTop2.row_number = lt_list.Count + 1;
              lt_list.Add(summaryCategoryTop2);
            }
          }
    ...
    public async IAsyncEnumerable<ProfitLossSummaryCategoryTop> SelectCategoryTopEnumerableAsync(
      object p_param)
    {
      ProfitLossSummaryCategoryTop summaryCategoryTop1 = this;
      UniOleDbRecordset rs = (UniOleDbRecordset) null;
      ...
      try
      {
        ...
        else if (await rs.IsDataReadAsync())
        {
          ...
          do
          {
            ...
            if (summaryCategoryTop2.GetFieldValues(rs))
            {
              summaryCategoryTop2.row_number = ++row_number;
              yield return summaryCategoryTop2;
            }
          }
          while (await rs.IsDataReadAsync());
        }
